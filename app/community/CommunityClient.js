...
const buddies = [
  { name: "Aisha M.",    track: "Python Programming", xp: 2480, emoji: "👩🏾‍💻" },
  { name: "Brian K.",    track: "Graphic Design",     xp: 2215, emoji: "🎨" },
  { name: "Neema W.",    track: "YouTube Automation", xp: 1980, emoji: "🎬" },
  { name: "Kevin O.",    track: "Web Development",    xp: 1720, emoji: "💻" },
]
          ...
          <div className="grid-4">
            {buddies.map((b, i) => (
              <div key={i} className={`card ${i % 2 ? "purple" : "lime"}`} style={{ padding: 22, textAlign: "center" }}>
                ...
                <div className="sticker ink" style={{ marginTop: 12, fontSize: 10 }}>⚡ {b.xp} XP</div>
            ...
            {lounges.map((l, i) => (
              <div key={i} className={`card ${l.color}`} style={{ padding: 24 }}>
                ...
                <button className="btn btn-ink" style={{ width: "100%" }}>
                  {l.live ? "Join Now" : "Set Reminder"} <span aria-hidden>→</span>
                </button>
              </div>
            ))}
          </div>
