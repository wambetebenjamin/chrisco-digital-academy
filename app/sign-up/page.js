...
export default function SignUp() {
  ...
  const { claimReferralBonus } = useGamification()
  ...
  useEffect(() => {
    try {
      ...
      if (ref) {
        setRefCode(ref)
        localStorage.setItem("chrisco_pending_ref", ref)
      } else {
        const saved = localStorage.getItem("chrisco_pending_ref")
    ...
    <AuthShell subtitle="Join thousands of young Africans building real, practical digital skills, and earning XP every day.">
      {success ? (
        <div style={{ textAlign: "center", padding: "20px 0" }}>
          <span
            style={{
              ...
              marginBottom: 18, border: "3px solid var(--ink)",
          ...
          <p style={{ color: "var(--body)", fontSize: 14.5, lineHeight: 1.7, marginBottom: 14 }}>
            We sent a confirmation link to{" "}
            <strong style={{ color: "var(--purple)" }}>{form.email}</strong>. Click it to activate your account,
            then sign in to start earning XP.
          </p>
          {refCode && (
            <div className="card lime" style={{ padding: 14, marginBottom: 20, textAlign: "left" }}>
              <div style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>
                Referral bonus pending
              </div>
              <div style={{ fontSize: 13 }}>
                Your 200 XP bonus unlocks when you start your first course.
              </div>
            </div>
          )}
        ...
        <>
          ...
          <p style={{ color: "var(--body)", fontSize: 14.5, marginTop: 12, marginBottom: refCode ? 14 : 28 }}>
          ...
          {refCode && (
            <div
              ...
              style={{ padding: "12px 16px", marginBottom: 18, display: "flex", alignItems: "center", gap: 10 }}
            >
              <span
                style={{
                  width: 32, height: 32, borderRadius: 10, background: "var(--ink)", color: "var(--lime)",
              ...
              <div style={{ fontSize: 12.5, fontWeight: 700, lineHeight: 1.3 }}>
                You were invited by a friend. You both earn <strong>200 bonus XP</strong> when you start your first course.
              </div>
            </div>
          )}
          ...
          <button
            ...
            style={{ cursor: loading ? "default" : "pointer", marginTop: 6 }}
          >
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 20, flexWrap: "wrap", justifyContent: "center" }}>
            ...
            <span className="sticker yellow" style={{ fontSize: 10 }}>XP and badges</span>
